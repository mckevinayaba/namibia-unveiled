...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your saved routes, itineraries and account for Beyond The Loop Namibia." },
    ],
  }),
  component: Profile,
});
...
function Profile() {
  return (
    <div className="pb-8">
      <PageHeader eyebrow="Profile" title="Your traveller." intro="Sign in to save routes, sync itineraries and access offline route files on the road." />
      ...
      <div className="container-wide mt-10 grid gap-8 md:grid-cols-12">
          ...
          <div className="overflow-hidden rounded-3xl border border-border bg-[color:var(--night)] p-6 text-[color:var(--bone)] md:p-8">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <p className="text-[11px] uppercase tracking-[0.2em] text-white/60">Guest traveller</p>
                <p className="truncate font-display text-2xl">Welcome back.</p>
              </div>
            </div>
            <p className="mt-5 text-sm text-white/70">
              Create an account to hold provisional bookings for 72 hours, sync your route files, and access on-trip support.
            </p>
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            {[
              ...
              <div key={s.k} className="rounded-2xl border border-border bg-card p-4 text-center">
                ...
                <p className="mt-1 text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{s.k}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="md:col-span-7">
              ...
              <li key={t}>
                <button className="flex w-full items-center gap-4 p-5 text-left hover:bg-secondary/60">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block truncate text-[12px] text-muted-foreground">{s}</span>
                  </span>
          ...
          <div className="mt-6 rounded-3xl border border-border bg-[color:var(--sand)] p-6">
            ...
            <Link to="/partner" className="mt-4 inline-flex text-sm font-medium underline underline-offset-4">
